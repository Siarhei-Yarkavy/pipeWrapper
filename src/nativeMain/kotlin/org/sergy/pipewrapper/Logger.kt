            ...
            memScoped {
                ...
                for (exe in Executable.entries) {
                    ...
                    if (logHandle == INVALID_HANDLE_VALUE || logHandle == null) {
                        fputs(
                            "Error creating log file for ${exe.name}! Error code ${GetLastError()}\n",
                            stderr
                        )
                        exitProcess(CREATE_LOGGER_FILE_FAILED)
                    }
    ...
    companion object {
        ...
        fun get(): ILogger =
            if (!isInitialized()) {
                ...
                fputs(
                    "ExitCode=$GET_LOGGER_INSTANCE_FAILED, Stack trace:\n" +
                ...
                if (!isInitialized()) {
                    object : ILogger {
                        override fun log(message: String) {
                            fputs(
                                "Warning! Logger is not available. Fallback to console stderr: $message",
                                stderr
                            )
                        }
